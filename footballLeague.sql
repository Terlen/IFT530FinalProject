...
-- Table Creation --
CREATE TABLE Team(TeamID INT PRIMARY KEY NOT NULL IDENTITY, Name VARCHAR(50), City VARCHAR(50), State VARCHAR(2));

CREATE TABLE Stadium(StadiumID INT PRIMARY KEY NOT NULL IDENTITY, StadiumName VARCHAR(50), City VARCHAR(50), State VARCHAR(2));

CREATE TABLE Player(PlayerID INT PRIMARY KEY NOT NULL IDENTITY, FirstName VARCHAR(25) NOT NULL, LastName VARCHAR(25) 
			NOT NULL, MiddleInitial VARCHAR(1), Height INT, Weight INT, DateOfBirth DATE);

CREATE TABLE Matchup(MatchupID INT PRIMARY KEY NOT NULL IDENTITY, StadiumID INT FOREIGN KEY REFERENCES Stadium(StadiumID),
			HomeTeamID INT FOREIGN KEY REFERENCES Team(TeamID), AwayTeamID INT FOREIGN KEY REFERENCES Team(TeamID), Date DATE,
			CONSTRAINT CHK_Team CHECK (HomeTeamID<>AwayTeamID))
